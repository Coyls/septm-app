  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-0.5 min-w-0 px-3 py-2.5 flex-1">
        ...
        {sublabel && (
          <span className="text-xs text-[var(--text-weak)] leading-tight">{sublabel}</span>
        )}
      </div>
      ...
      {image ? (
        <div className="relative shrink-0 h-full w-24">
          ...
          <div className="absolute inset-y-0 left-0 w-6 z-10 bg-gradient-to-r from-[var(--card)] to-transparent" />
          <Image
            src={image}
            alt={label}
            fill
