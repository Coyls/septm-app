            ...
            <TableBody>
              {data.map((e) => (
                <TableRow key={e.extensionId}>
                  ...
                  <TableCell className="text-right">{formatScore(Math.round(e.averageScorePerPlayer))}</TableCell>
                  <TableCell className="text-right">{e.averageWinnerCount.toFixed(1)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
