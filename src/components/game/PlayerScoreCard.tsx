...
interface PlayerScoreCardProps {
  ...
  onChange: (playerId: string, field: "coins" | PointTypeId, value: number) => void
}

export function PlayerScoreCard({ player, allowedPointTypes, onChange }: PlayerScoreCardProps) {
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <Badge variant="outline" className="text-sm font-bold">
            Total : {Object.values(player.points).reduce((a, b) => a + b, 0) + coinsPoints} pts
          </Badge>
        </div>
      </CardHeader>
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2">
            <Input
              ...
              value={player.coins || ""}
              onChange={(e) =>
                onChange(player.playerId, "coins", parseInt(e.target.value) || 0)
              }
              className="w-24"
              placeholder="0"
            />
