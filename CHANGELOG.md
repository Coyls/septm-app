...
- Score page: replaced `useEffect` + `setState` initialization pattern with `useMemo` (base scores) + `useState` (user overrides), eliminating cascading renders and the `set-state-in-effect` lint error
